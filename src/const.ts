...
export const NetworkSettings = {
  ...
  chainId: "0",
  nameSpace: "user", // user --> chain0; free --> chain1
  module: "pacty-parrots",
};

export const createAPIHost = () =>
  `https://${NetworkSettings.apiEndpoint}/chainweb/0.0/testnet04/chain/${NetworkSettings.chainId}/pact`;
...
export async function FetchPactLocalRaw(cmd: TodoType) {
...
export async function FetchPactLocal(command: string, parameters?: string) {
  const pactCode = `(${NetworkSettings.nameSpace}.${
    NetworkSettings.module
  }.${command} ${parameters ? JSON.stringify(parameters) : ""})`;
  const { result } = await Pact.fetch.local(
    {
      pactCode,
      keyPairs: dumKeyPair,
